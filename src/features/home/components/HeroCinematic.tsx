 ...
 */
/** Entrance delay that waits for the silk intro (0s when it does not play). */
const delay = (ms: number) => ({ animationDelay: `calc(var(--intro-offset, 0s) + ${ms}ms)` });
  ...
  return (
    <section className="relative isolate flex min-h-[92svh] items-end overflow-hidden bg-crimson-900 text-ivory-50">
      <Image
        ...
        style={{ animationDelay: 'var(--intro-offset, 0s)' }}
      />
      ...
      <div className="container-editorial w-full pt-40 pb-10 md:pb-14">
        <p className="eyebrow animate-fade-up text-marigold-300" style={delay(0)}>
          {hero.eyebrow}
        </p>
        <h1
          className="display-hero animate-fade-up mt-5 max-w-3xl font-light text-ivory-50"
          style={delay(120)}
        >
        ...
        <p
          className="animate-fade-up mt-6 max-w-md text-[0.9375rem] leading-relaxed text-ivory-100/80"
          style={delay(240)}
        >
          {hero.body}
        </p>

        <div className="animate-fade-up mt-9 flex flex-wrap items-center gap-6" style={delay(360)}>
          <Link
        ...
        <ul
          style={delay(480)}
          className="animate-fade-up mt-14 flex flex-wrap gap-x-10 gap-y-3 border-t border-ivory-50/15 pt-6 text-xs text-ivory-100/75"
        >
